...
class TurbulenceDataset(Dataset):
    ...
    def __init__(
        ...
        dataDirs: List[str],
        filterTop: List[str],
        excludeFilterTop: bool = False,
        filterSim: List[Tuple[int, int]] = [],
        excludefilterSim: bool = False,
        filterFrame: List[Tuple[int, int]] = [],
        sequenceLength: List[Tuple[int, int]] = [],
        randSeqOffset: bool = False,
        simFields: List[str] = [],
        simParams: List[str] = [],
        printLevel: str = "none",
        logLevel: str = "sim",
        ...
        assert len(filterSim) in [
            0,
            1,
            len(filterTop),
        ], "Sim filter is not set up correctly. Use len=0 for all; len=1 for the same everywhere, len=len(filterTop) to adjust for each top filter"
        assert len(filterFrame) in [
            1,
            len(filterTop),
        ], "Frame filter is not set up correctly. Use len=1 for the same everywhere, len=len(filterTop) to adjust for each top filter"
        assert len(sequenceLength) == len(
            filterFrame
        ), "Sequence length is not set up correctly, it should match the frame filter."
        if excludeFilterTop:
            assert (
                len(filterSim) <= 1
            ), "Excluded top filter and adjust sim filtering is not supported!"
            assert (
                len(filterFrame) <= 1
            ), "Excluded top filter and adjust frame filtering is not supported!"
        ...
        for dataDir in dataDirs:
            ...
            for topDir in topDirs:
                ...
                if printLevel == "top":
                    self.summaryPrint += [
                        "Top folder loaded: " + simDir.replace(dataDir + "/", "")
                    ]
                if logLevel == "top":
                    self.summaryLog += [
                        "Top folder loaded: " + simDir.replace(dataDir + "/", "")
                ...
                for sim in sims:
                    ...
                    if len(filterSim) > 0:
                        ...
                        if len(filterSim) == 1:
                            if type(filterSim[0]) is tuple:
                                inside = (
                                    simNum >= filterSim[0][0]
                                    and simNum < filterSim[0][1]
                                )
                            ...
                            if type(filterSim[match]) is tuple:
                                inside = (
                                    simNum >= filterSim[match][0]
                                    and simNum < filterSim[match][1]
                                )
                    ...
                    if printLevel == "sim":
                        self.summaryPrint += [
                            "Sim loaded: " + currentDir.replace(dataDir + "/", "")
                        ]
                    if logLevel == "sim":
                        self.summaryLog += [
                            "Sim loaded: " + currentDir.replace(dataDir + "/", "")
                    ...
                    minFrame = (
                        filterFrame[0][0]
                        if len(filterFrame) == 1
                        else filterFrame[match][0]
                    )
                    maxFrame = (
                        filterFrame[0][1]
                        if len(filterFrame) == 1
                        else filterFrame[match][1]
                    )
                    seqLength = (
                        sequenceLength[0][0]
                        if len(sequenceLength) == 1
                        else sequenceLength[match][0]
                    )
                    seqSkip = (
                        sequenceLength[0][1]
                        if len(sequenceLength) == 1
                        else sequenceLength[match][1]
                    )
                    for seqStart in range(minFrame, maxFrame, seqLength * seqSkip):
                        validSeq = True
                        for frame in range(
                            seqStart, seqStart + seqLength * seqSkip, seqSkip
                        ):
                            ...
                            for field in self.simFields:
                                currentField = os.path.join(
                                    currentDir, "%s_%06d.npz" % (field, frame)
                                )
                        ...
                        if printLevel == "full":
                            self.summaryPrint += [
                                "Frames %s loaded: %s/%s_%06d-%06d(%03d).npz"
                                % (
                                    "-".join(self.simFields),
                                    currentDir.replace(dataDir + "/", ""),
                                    "-".join(self.simFields),
                                    seqStart,
                                    seqStart + seqLength * (seqSkip - 1),
                                    seqSkip,
                                )
                            ]
                        if logLevel == "full":
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if self.randSeqOffset:
            ...
            if (
                seqStart + offset >= self.filterFrame[0][0]
                and seqEnd + offset < self.filterFrame[0][1]
            ):
        ...
        with open(os.path.join(basePath, "src", "description.json")) as f:
            ...
            if "rey" in self.simParams and "mach" in self.simParams:
                simParameters = np.stack(
                    [loadedParams["Reynolds Number"], loadedParams["Mach Number"]],
                    axis=1,
                )
        ...
        for frame in range(seqStart, seqEnd, seqSkip):
            for field in self.simFields:
                loadedArr = np.load(
                    os.path.join(basePath, "%s_%06d.npz" % (field, frame))
                )["arr_0"]
        ...
        if type(simParameters) is not dict:
            ...
            if vel.ndim == 4:
                ...
                simParExpanded = np.repeat(
                    np.repeat(simParExpanded, vel.shape[2], axis=2),
                    vel.shape[3],
                    axis=3,
                )
            elif vel.ndim == 5:
    ...
    def printDatasetInfo(self):
        if self.transform:
            s = "%s - Data Augmentations: %s\n" % (
                self.name,
                len(self.transform.p_d.augmentations),
            )
            s += "\tactivate augmentations: [%s]\n" % ", ".join(
                self.transform.p_d.augmentations
            )
            if self.transform.crop:
                s += "\tcrop settings: outputSize (%d, %d)\n" % (
                    self.transform.outputSize[0],
                    self.transform.outputSize[1],
                )
    ...
    def getFilterInfoString(self) -> str:
        ...
        s += "\tfilterTop: %s  exlude: %s\n" % (
            str(self.filterTop),
            self.excludeFilterTop,
        )
        s += "\tfilterSim: %s  exlude: %s\n" % (
            str(self.filterSim),
            self.excludefilterSim,
        )
