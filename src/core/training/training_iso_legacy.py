...
if __name__ == "__main__":
    ...
    #             transTrainUnroll=True, transTargetFull=True, maxInputLen=25)
    # pretrainPath = ""
    ...
    # p_me = ModelParamsEncoder(arch="skip", pretrained=False, encWidth=32, latentSize=64)
    ...
    #             transTrainUnroll=True, transTargetFull=False, maxInputLen=25)
    # pretrainPath = ""
...
def train(modelName:str, trainSet:TurbulenceDataset, testSets:Dict[str,TurbulenceDataset],
    ...
    for shortName, testSet in testSets.items():
        ...
        testHistory = LossHistory(shortName, testSet.name, logger.tfWriter, len(testLoader),
                                    10, 10, printInterval=0, logInterval=0, simFields=p_d.simFields)
